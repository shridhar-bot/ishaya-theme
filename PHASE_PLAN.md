...
| Task | Status |
|---|---|
| Build Integrity Gate (`QA_QC_STANDARDS.md` §0) passing | Done |
| Full QA pass against a store with real products | Not started |
| Performance audit (PageSpeed/Lighthouse) | Not started |
| Cross-browser + cross-device testing | In progress (Chrome × 4 breakpoints × 2 themes done; Safari/Firefox/Edge pending) |
...
- 2026-09-13: **Light-mode logo + PDP legend spacing.** Client reported the uploaded logo vanishing in light mode (dark artwork on a light pill). Added a `logo_light` theme setting (Brand group; `logo` relabelled "dark mode") and `snippets/brand-logo.liquid`, which emits the light artwork first and the dark second; `assets/critical.css` (`.ishaya-logo`) shows only the one matching `:root[data-theme]`, or `prefers-color-scheme` in system mode, so the swap is pure CSS with no flash. Header desktop, mobile pill and both footer slots now render through the snippet. Selectors are doubled-class so they out-rank the per-slot `display: block` rules — the first pass tied on specificity and showed both logos in dark mode. `.ishaya-pdp__option-legend` gained `margin-bottom: 8px`. Verified on the live dev theme: dark → only dark logo, toggle → only light logo, toggle back → dark. 58 files, 0 offenses; gate clean.
- 2026-09-13: **PDP buy box rebuilt to client reference.** Client supplied a screenshot of a reference buy box (structure only, per brief §2): `SELECT SIZE` legend with a `GUIDE` link, equal-width size tiles, stacked full-width Add to bag (outlined) and Buy now (solid), and a tabbed info card (Description / Details / Size & fit / Care / Shipping & return) with clamped copy and a `VIEW MORE` toggle. Implemented in `sections/product.liquid`: legends now read `Select <option>` with the value muted; value pills are an auto-fit equal-width grid with input radius; quantity stepper hidden behind a new `show_quantity` setting (off by default); Add to bag switched to the outlined secondary style and the dynamic checkout button styled as the solid primary (label stays Shopify's `Buy it now` — not overridable); the four hardcoded accordions replaced by the tabbed card, with Details / Size & fit / Care / Shipping & return now `richtext` section settings (old copy carried over as defaults, empty field hides its tab). `View more` only appears when the body actually overflows (measured on tab switch and resize) and the clipped tail fades into the card. Mobile: tabs become one swipeable row; `min-width: 0` on the column/card stops the nowrap row from widening the page (caught by a 375px overflow check). Also `white-space: nowrap` on breadcrumb links. Verified on the live dev theme at 1400px dark + light and 375px: tabs switch, View more expands/collapses, no horizontal scroll. 58 files, 0 offenses.
- 2026-09-15: **Product card buy controls.** Client asked for a larger, more visible price on every product card with Add to bag beside it and Buy now beneath. `snippets/product-card.liquid`: the floating `+ ADD` pill on the image is gone; the price row is now price (0.95rem/700) + an outlined `ADD TO BAG` button (single variant → delegated quick add; multi-variant → opens the existing variant tray; sold out → inert `SOLD OUT` chip), followed by a full-width solid `BUY NOW` — a cart permalink (`/cart/<variant>:1`, straight to checkout) for single-variant products, the product page otherwise. Both controls sit at `z-index: 2` above the stretched title-link overlay. Removed the now-unused `.ishaya-product-card__quick-add` CSS. Verified on the live dev theme at 1400px and 375px with hit-tests on both buttons; the wishlist drawer's own Add to bag is unaffected. **Note:** every product on the dev store currently reports `available: false` (including 'Inventory Not Tracked'), so in-stock markup had to be simulated in the browser — that is a store/inventory/market state, not a theme defect.
- 2026-09-15: **Comment sweep.** On client direction, removed explanatory ("why") comments from the files touched this week — Liquid `{% comment %}` blocks, `{% # %}` lines, CSS/JS block comments and full-line `//` notes — and collected all 112 of them in `CODE_NOTES.txt`, grouped by theme directory and file with the original line number and the first line of code each note described. LiquidDoc `{% doc %}` headers (required by AGENTS.md), `<!-- -->` structural labels and `/* --- */` section dividers were kept. Also fixed a pre-existing PDP Liquid error on variants without featured media (see QA log). `node --check` on both JS files, `theme check` (58 files, 0 offenses), integrity gate and a live render of home / collection / product all clean.
- 2026-09-18: **Final QA + release package.** Removed Skeleton scaffold sections and unused SVGs (56 files now). 88-render automated sweep clean (see QA log). Packaged `releases/ishaya-theme-2026-09-18.zip` via `shopify theme package` for manual upload. Note: `config/settings_data.json` is empty in the repo and on origin, so a fresh upload starts with default theme settings — logo (dark + light), colours and default theme mode must be re-set in the editor, or deploy via GitHub sync instead, which keeps the live theme's settings.
